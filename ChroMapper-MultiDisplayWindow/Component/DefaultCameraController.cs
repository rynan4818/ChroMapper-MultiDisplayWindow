...
namespace ChroMapper_MultiDisplayWindow.Component
{
    public class DefaultCameraController : MonoBehaviour
    {
        ...
        public float y;
        public float z;
        public Camera[] targetCamera { get; set; } = { null, null, null, null };
        private static readonly Type[] actionMapsDisableDefaultCamera = { typeof(CMInput.ICameraActions), typeof(CMInput.ITimelineActions), typeof(CMInput.IPlaybackActions) };
        ...
        private void LateUpdate()
        {
            ...
            if (EventSystem.current != null && EventSystem.current.IsPointerOverGameObject())
                return;
            if (MultiDisplayController.activeWindowNumber == -1)
                return;
            if (MultiDisplayController.activeWindowNumber == 0 && targetCamera[MultiDisplayController.activeWindowNumber] == null)
            {
                ...
                {
                    ActionMapsController.EnableAction(actionMapsDisableDefaultCamera);
            ...
            else if (targetCamera[MultiDisplayController.activeWindowNumber] != null || MultiDisplayController.activeWindowNumber > 0)
            {
                if (!disableMainDefaultCamera)
                {
                    ActionMapsController.DisableAction(actionMapsDisableDefaultCamera);
